...
# Note that decoding is disabled for Luganda
...
L=acholi
LANGUAGES="acholi" # Separate these with single spaces if specifying >1 langs
...
for L in $LANGUAGES; do
  ...
  # Change for Luganda: no need to make phoneme-based dict
  for D in dict_grapheme; do
    ...
    # Change for Luganda: LMs do not exist, so no need to format
...
exp="trimmed_lang_grapheme"
mono_train_set="train_dev_trimmed"
tri1_train_set=$mono_train_set
tri2a_train_set=$mono_train_set
tri2b_train_set=$mono_train_set
tri3b_train_set=$mono_train_set
test_set="eval_trimmed"

train_lang="lang"
test_lang="lang_test"

# Change for Luganda: tweak to split the data and get features
mkdir -p data/$L/all_trimmed
cp data/$L/local/data_trimmed/* data/$L/all_trimmed

for L in $LANGUAGES; do
  # Feature creation
  mfccdir=mfcc/$L
  for x in all_trimmed; do
    (
...
utils/subset_data_dir_tr_cv.sh --cv-spk-percent 5 data/$L/all_trimmed \
  data/$L/train_trimmed data/$L/eval_trimmed


# # Trying a lower training set for mono
# utils/subset_data_dir.sh --shortest data/acholi/$mono_train_set 1000 data/acholi/${mono_train_set}_1kshort
# mono_train_set="${mono_train_set}_1kshort"
#
# # Train a simple mono models
# for train_lang in lang lang_nosp lang_pos_ind lang_nosp_pos_ind; do
#   exp="trimmed_${train_lang}_1kshort"
...
# # Note: the lm_suffix includes _sri if re-created using SRILM
# for L in $LANGUAGES; do
...
# # Trying a lower training set for triphones
# utils/subset_data_dir.sh --shortest data/acholi/$tri1_train_set 2000 data/acholi/${tri1_train_set}_2kshort
# tri1_train_set="${tri1_train_set}_2kshort"
# exp_old=$exp
# exp="trimmed_lang"
...
#       $graph_dir
#     num_test_spk=$(cat data/acholi/$test_set/spk2utt | wc -l)
#     [ $num_test_spk -lt $max_num_jobs ] \
#       && dec_nj=$num_test_spk || dec_nj=$max_num_jobs
#     steps/decode.sh --nj $dec_nj --cmd "$decode_cmd" \
#       $graph_dir data/$L/$test_set \
