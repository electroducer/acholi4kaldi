...
# Borrowed mainly from the rm counterpart
...
exp= # Optional experiment suffix
train_lang=lang # Optionally change the default lang dir
...
if [ $stage -le 0 ]; then
  ...
  for i in $(seq 0 $[num_langs-1]); do
    ...
    rm $tgt_dir/{feats,cmvn}.scp # remove features,
...
dir=exp/dnn4g-multilingual${num_langs}-$(echo $lang_code_csl | tr ',' '-')-${nnet_type}${exp}
[ ! -e $dir ] && mkdir -p $dir
...
if [ $stage -le 1 ]; then
  ...
  for i in $(seq 0 $[num_langs-1]); do
    code=${lang_code[$i]}
    ali=${ali_dir[$i]}
    # utt suffix added by 'awk', (removed this, since not using suffix)
  ...
  if [ $num_langs = 1 ]; then
    cp $dir/ali-post/$code.ark $dir/ali-post/combined.ark
    cp $dir/ali-post/$code.scp $dir/ali-post/combined.scp
  else
  ...
  case $nnet_type in
    ...
    $cuda_cmd $dir/log/train_nnet.log \
      steps/nnet/train.sh --learn-rate 0.008 \
        ...
        ${data_tr90} ${data_cv10} data/${lang_code[0]}/$train_lang ${ali_dir[0]} ${ali_dir[0]} $dir
    ;;
    sbn)
